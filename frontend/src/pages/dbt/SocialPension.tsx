...
export default function SocialPension() {
  ...
  const verify = async () => {
    if (!createdId) return
    await client.post(`/dbt/verify/${createdId}`)
    setCreatedId(null)
  }
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 rounded-xl bg-white p-6 shadow-sm md:grid-cols-2">
        ...
        <div className="md:col-span-2 flex gap-3">
          ...
          <button type="button" disabled={!createdId} onClick={verify} className="rounded-md border border-[#01696f] px-4 py-2 font-semibold text-[#01696f] disabled:opacity-50">Run mock verification</button>
        </div>
      </form>
    </div>
  )
